...
//     // die;
// }
// $sqlCount = "SELECT count(id) as likes_num FROM `likescomment`";
// $resCount = mysqli_query($con, $sqlCount);
// if ($resCount) {
//     $row = mysqli_fetch_assoc($resCount);
//     // var_dump($row);
//     // die;
//     $likeNum = $row["likes_num"];
//     // $_SESSION["likeNumber"] = $likeNumber;
//     // var_dump($_SESSION["likeNumber"]);
//     // echo $_SESSION["likeNumber"];
//     // die;
            ...
            <div class="card shadow-sm p-4 col-md-10 col-lg-8 col-xl-7 mx-auto">
                <div class="row gx-4 gx-lg-5 justify-content-center">
                    <div class="col-md-10 col-lg-8 col-xl-7">
                        ...
                        <?php  
                        $post_id=$post["id"];
                        $sqlLikeNum = "SELECT COUNT(*) as like_Num FROM `likes` WHERE post_id = $post_id";
                        $resultLikeNum = mysqli_query($con, $sqlLikeNum);
                        $likeData = mysqli_fetch_assoc($resultLikeNum);
                        $likeNum = $likeData['like_Num'] ?? 0; 
                        ...
                        <form action="./controller/posts/likeOnPost.php" method="POST" class="mt-2">
                            ...
                            <button type="submit" class="btn btn-outline-primary">Like <?= $likeNum; ?></button>
                        </form>
                        ...
                        <div class="comments-container">
                            ...
                            if (!empty($comments)):
                                foreach ($comments as $comment):
                                    ...
                                    <?php  
                                    $sqlLikeCount = "SELECT COUNT(*) as like_count FROM `likescomment` WHERE comment_id = $comment_id";
                                    $resultLikeCount = mysqli_query($con, $sqlLikeCount);
                                    $likeData = mysqli_fetch_assoc($resultLikeCount);
                                    $likeCount = $likeData['like_count'] ?? 0; 
                                    
                                    ?>
                                    <form action="./controller/comments/likeOnComment.php" method="POST" class="mt-2">
                                        <input type="hidden" name="comment_id" value="<?= $comment_id; ?>">
                                        <input type="hidden" name="user_id" value="<?= $_SESSION["auth"]["id"]; ?>">
                                        <input type="hidden" name="page" value="home">
                                        <button type="submit" class="btn btn-outline-primary">Like <?= $likeCount; ?></button>
                                    </form>
                                <?php
                                endforeach;
                            else:
